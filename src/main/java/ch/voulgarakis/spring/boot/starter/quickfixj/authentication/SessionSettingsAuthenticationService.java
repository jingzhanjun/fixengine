...
public class SessionSettingsAuthenticationService implements AuthenticationService {
    ...
    private void authenticate(SessionID sessionID, Message message, String sessionSettingsProperty, StringField field)
        ...
        try {
            String value = null;
            if(!StringUtils.isBlank(sessionSettingsProperty)) {
            	if(!sessionSettingsProperty.equals("Username")&&!sessionSettingsProperty.equals("Password")) {
            		value=sessionSettings.getString(sessionID, sessionSettingsProperty);
            	}
            }
